...
<label className="text-sm font-medium">Batch Size</label>
		<input value={config.batchSize} onChange={(e) => setConfig({ batchSize: Number(e.target.value) })} type="number" className="border p-2 rounded w-full" />
</div>
<div className="space-y-2">
<label className="text-sm font-medium">Preprocessing Mode</label>
		<select value={config.preprocessing} onChange={(e) => setConfig({ preprocessing: e.target.value })} className="w-full p-2 border rounded bg-muted">
